...
$result = mysqli_query($link, "SELECT TrackingNumber FROM orders WHERE CarrierName='USPS' and TrackingStatus<>'Delivered'  and TrackingNumber<>'' and ShipDate>='2018-12-12' ");
$xml = "";
...
print ('<div>');	
	for ($x = 0; $x < count($TrackInfo); $x++)
		...
		updateStatus($TrackingStatus, $TrackingNumber,$updateTime, $link);
...
function updateStatus($TrackingStatus, $TrackingNumber,$updateTime, $link)
	{
	$resInsert = mysqli_query($link, "update orders set TrackingStatus='$TrackingStatus', UpdateTime='$updateTime'  WHERE TrackingNumber ='$TrackingNumber' "); // insert one row into new table
	if ($resInsert)
		{
			print ("update orders set TrackingStatus='$TrackingStatus', UpdateTime='$$updateTime'  WHERE TrackingNumber ='$TrackingNumber'" . "<br />");

...
$sys_log = $updateTime . "\r\n";
sys_trackingStatus($updateTime,$ShipDate,$link,$sys_log);

function sys_trackingStatus($updateTime,$ShipDate,$link,&$sys_log)
	{
		$count=0;
		...
		$result = mysqli_query($link, "SELECT * FROM orders o WHERE o.TrackingStatus= 'In-Transit' AND CarrierName = 'fedex' AND `shipclass` not LIKE '%SELLER PRIME%' and ShipDate<'$ShipDate'  and StoreType<>'RMA'  ORDER BY ShipDate DESC");
		while($row=mysqli_fetch_object($result))
		{
			...
			curl($TrackingNumber,$TxnId,$updateTime,$link,$sys_log);
			sleep(0.5);
			echo $count++.'<br>';
		}
		$sys_log.= 'fedex_delivery_tracking_standard'.$count.'Done \r\n"';
...
function curl($TrackingNumber,$TxnId,$updateTime,$link)
{
		...
		$path_to_wsdl = "C:\\xampp\\htdocs\\Libraorderstatus\\sys_log\\TrackService_v16.wsdl";
...
mysqli_close($link);
$t = time();
$filePath = "C:\\xampp\\htdocs\\Libraorderstatus\sys_log\\logs\\$t._fedex.log";

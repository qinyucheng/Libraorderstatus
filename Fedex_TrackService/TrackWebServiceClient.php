...
sys_trackingStatus($updateTime,$link);

function sys_trackingStatus($updateTime,$link)
	{
		$count=0;
		...
		$result = mysqli_query($link, "SELECT * FROM orders WHERE TrackingStatus <>'Delivered' and CarrierName='fedex' and TrackingNumber<>'' and ShipDate<='2018-12-13' and ShipDate>='2018-12-12' ");
						...
						if(property_exists($response, 'CompletedTrackDetails'))
						{
							...
							if($TrackingStatus=='Delivered' || $TrackingStatus=='delivered')
							{
								...
								else 
								{
									...
									updateStatus($TrackingStatus,$TxnId,$updateTime,$link);
							echo  "<br>--------------------------------------------<br>";
...
function updateStatus($TrackingStatus,$TxnId,$updateTime,$link)
{
	
	$resInsert = mysqli_query($link, "update orders set TrackingStatus='$TrackingStatus', UpdateTime='$updateTime'  WHERE TxnId ='$TxnId' "); // insert one row into new table
	if ($resInsert)
		{
		...
		{
		print ("update orders set TrackingStatus='$TrackingStatus',UpdateTime='$updateTime'  WHERE TxnId ='$TxnId'". "<br />");
		}		
}
